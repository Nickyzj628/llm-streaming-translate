...
**Note**: This project uses the system font stack rather than loading custom font files. The negative letter-spacing and tight tracking principles described below are applied to the system font stack (`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`) for a similarly compressed, engineered feel.
...
- **System font stack** with tight negative letter-spacing (`letter-spacing: -0.03em` on headings) — text as compressed infrastructure.
...
- **Primary**: System font stack (`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif`) | **Monospace**: `"SF Mono", "Fira Code", Roboto Mono, monospace`
- **Ligatures & Spacing**: `"liga"` enabled globally where supported. Use `"tnum"` (`font-variant-numeric: tabular-nums`) for number columns/comparisons.
...
- **Don't** use positive letter-spacing on headings — the tight negative tracking is intentional.
...
- **Framework**: SolidJS + WXT + SCSS Modules
- **Typography**: System font stack, tight negative letter-spacing on headings, tabular nums for data.
- **Spacing/Layout**: Tailored Flex/Grid, `min-w-0` on flex children for text truncation.
- **State**: Signal-driven reactive state (SolidJS primitives).
